...
def _main():
    ...
    if resource_type == ResourceType.VIDEO:
        ...
        obj: bangumi.Episode
        print(Fore.GREEN + "INF: 视频 AID: ", obj.get_aid())
        print(Fore.GREEN + "INF: 视频 EPID: ", obj.get_epid())
        pages_data = sync(obj.get_pages())
        print(Fore.GREEN + "INF: 正在获取下载地址")
        ...
        if FFMPEG != "#none":
            if "dash" in download_url.keys():
                ...
                if audio_url == None:
                    ...
                    exit()
                print(Fore.GREEN + "INF: 开始下载视频")
                video_path = _download(video_url, "video_temp.m4s", vinfo["title"] + f" - {obj.get_bvid()}(番剧 EP{obj.get_epid()}) - 视频")
                print(Fore.GREEN + "INF: 开始下载音频")
                audio_path = _download(audio_url, "audio_temp.m4s", vinfo["title"] + f" - {obj.get_bvid()}(番剧 EP{obj.get_epid()}) - 音频")
                print(Fore.GREEN + "INF: 下载视频完成 开始混流")
                print(Fore.RESET)
                if PATH == "#default":
                    PATH = vinfo["title"] + f" - {obj.get_bvid()}(番剧 EP{obj.get_epid()}).mp4"
                os.system(f'{FFMPEG} -i video_temp.m4s -i audio_temp.m4s -vcodec copy -acodec copy "{PATH}"')
                os.remove("video_temp.m4s")
                os.remove("audio_temp.m4s")
                print(Fore.GREEN + f"INF: 混流完成(或用户手动取)")
                ...
                PATH_FLV = PATH.rstrip(".mp4")
                print(Fore.GREEN + "INF: 开始下载视频")
                video_path = _download(video_audio_url, PATH_FLV, vinfo["title"] + f" - {obj.get_bvid()}(番剧 EP{obj.get_epid()})")
                turn_format = input(Fore.BLUE + "Y/N: 是否转换成 MP4 视频(默认转换): ")
                if turn_format.upper() == "N":
                    pass
                else:
            ...
            if "dash" in download_url.keys():
                ...
                if PATH == "#default":
                    PATH = vinfo["title"] + f" - {obj.get_bvid()}(番剧 EP{obj.get_epid()}).mp4"
                print(Fore.GREEN + "INF: 开始下载视频")
                video_path = _download(video_url, "视频 - " + PATH, vinfo["title"] + f" - {obj.get_bvid()}(番剧 EP{obj.get_epid()}) - 视频")
                print(Fore.GREEN + "INF: 开始下载音频")
                audio_path = _download(audio_url, "音频 - " + PATH, vinfo["title"] + f" - {obj.get_bvid()}(番剧 EP{obj.get_epid()}) - 音频")
                print(Fore.GREEN + "INF: BiliDown 下载完成")
                print(Fore.GREEN + "视频地址：(" + "视频 - " + PATH + ")")
                print(Fore.GREEN + "音频地址：(" + "音频 - " + PATH + ")")
                exit()
