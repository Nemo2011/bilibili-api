...
def _require_file_type(filename: str, filetype: str):
    ...
    if filename.rstrip(filetype) == filename:
        ...
        print(Fore.YELLOW + f"WRN: 您可以使用参数 --disable-filetype-check 忽略或")
        repair = input(Fore.BLUE + f"Y/N: 是否自动修复: ")
        if repair.upper() == "N":
            return filename
        else:
...
def _download_video(obj: video.Video, now_file_name: str):
    ...
    if len(pages_data) > 1:
        ...
        while download_p == None:
            p = input(Fore.BLUE + "NUM: 请输入想要下载的分 P 【或者在数字两边扩上括号，获取此分 P 的信息，如 P(2)】: P")
