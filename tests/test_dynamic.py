...
async def test_a_send_dynamic():
    ...
    print('测试立即发送图片动态')
    resp = await dynamic.send_dynamic(text="测试立即发送图片动态", images_path=['./design/logo.png'], credential=credential)
    dyid.append(resp["dynamic_id"])
    print(resp)
    ...
    print('测试定时发布图片动态')
    resp = await dynamic.send_dynamic(text="测试定时发送图片动态", send_time=date, images_path=['./design/logo.png'], credential=credential)
    print(resp)
    draft_ids.append(resp['draft_id'])
