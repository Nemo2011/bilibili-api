...
async def test_set_favorite():
    media_list_id = 1195349595
    data = await video.set_favorite([media_list_id])
    await asyncio.sleep(0.5)
    await video.set_favorite(del_media_ids=[media_list_id])
    return data
...
async def test_del_tag():
    try:
        tag_id = 4949760
        data = await video.delete_tag(tag_id)
        return data
    except ResponseCodeException as e:
        # 16070  只有 UP 才能添加
        if e.code != 16070:
